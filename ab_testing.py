...
# Sampling (Örnekleme)
############################
# 0 ile 80 arasında 10000 tane sayı oluşturuyoruz rastgele
populasyon = np.random.randint(0, 80, 10000)
populasyon.mean()

# neden örneklem ile çalışıyoruz?
# diyelimki 10000 kişilik bir ilçede yaşayan insanların yaş ortalamasını bulmaya çalışıyoruz.
# örnek teorisi der ki, 10000 kişiyi iyi temsil eden  bir alt küme seç rastgele ve yansız olsun.
# Bu durumda 10000 kişiyi gezmeden bir genelleme yapma şansı bulmuş oluruz.

np.random.seed(115)
# numpy random seç populasyondan 100tane
orneklem = np.random.choice(a=populasyon, size=100)
orneklem.mean()

# başka örneklemler de almak istersek:
np.random.seed(10)
...
# bu 10 örneklemin ortalamasını alsak:
(orneklem1.mean() + orneklem2.mean() + orneklem3.mean() + orneklem4.mean() + orneklem5.mean() +
 orneklem6.mean() + orneklem7.mean() + orneklem8.mean() + orneklem9.mean() + orneklem10.mean() ) / 10

# Dikkat!
# Örneklem sayısı arttığında bu örneklem dağılımına ilişkin ortalama da
# populasyana (ana kitlenin ortalamasına) yakınlaşacaktır.
...
# total_bill için güven aralığı
sms.DescrStatsW(df["total_bill"]).tconfint_mean()

# tip değişkeni için güven aralığı
...
# yaş değişkenindeki eksik değerlerden kurtuluyoruz
# daha sonra güven aralığını hesaplıyoruz.
sms.DescrStatsW(df["age"].dropna()).tconfint_mean()

# fare değişkenindeki eksik değerlerden kurtuluyoruz.
# daha sonra güven aralığını hesaplıyoruz.
# fare: bilet ücretleri
...
df =sns.load_dataset("tips")
df.head()
# Amaç: Verilen bahşiş ile ödenen ücret arasında korelasyon var mı?
# Beklenti normalde ödenen hesap arttıkça bahşiş artar.
# veri setinde yemeğin toplam fiyatına bahşiş de dahil old için
# toplam ücretten bahşişi çıkarırsak yemeğin gerçek fiyatı bulunur.

df["total_bill"] = df["total_bill"] - df["tip"]

df.plot.scatter("tip", "total_bill")
plt.show(block=True)
# pozitif yönlü ilişki var.

# bahşiş ile toplam ücret arasındali korelasyon değeri
df["tip"].corr(df["total_bill"])

# Yorum:
# bahşiş ile toplam hesap arasında pozitif yönlü orta şiddetin biraz üzerinde
# bir ilişki var. Dolayısıyla ödenen hesap miktarı arttıkça bahşiş de artacaktır.
...
# smoker kırılımında total_bill in ortalaması
...
# Sonuç: Normallik varsayımı her iki durum için de sağlanmadı.
# Parametrik test kullanılamaz.
# Ama biz görelim diye normalmiş gibi davranıp varyans homojenliğine bakıcaz.
...
# varyans homojenliği testini gerçekleştirmek için levene testi kullanılır.
...
# cinsiyete göre kırılan veride yaşların ortalaması
...
# outcome değeri 1 ise kişi diyabet, 0 ise diyabet değil
...
# 1.grubun başarı sayısı 300, 2.grubun başarı sayısı 250
gozlem_sayilari = np.array([1000, 1100])
# 1.grubun gözlem sayısı 1000, 2.grubun gözlem sayısı 1100
...
# shape[0] da gözlem sayısı, shape[1] de değişken sayısı vardır.
...
# günlere göre ödenen ücretlerin ortalamaları
...
# Varsayım sağlanmıyorsa ikiden fazla grup ortalaması kıyaslama testi,
# non-parametrik kruskal testi kullanılır.

# varsayımın sağlandığını varsayalım yöntemi öğrenmek için:
...
# ikili karşılaştırmanın farklılığının hangi gruptan kaynaklandığını bulmak için,
# statsmodels içindeki çoklu karşılaştırma tukey testi ni kullanıyoruz:
